 ...
 * terms.
 */
package com.uk.sec.privilege.framework.privilege.util;
...
public class PFUtils {
    ...
    public enum SplitSignal {
        SPLIT_SIGNAL_AIT("@@@"), SPLIT_SIGNAL_LOGIC_AND("&&"), // 逻辑与
        SPLIT_SIGNAL_LOGIC_OR("||"); // 逻辑或
        
        private final String signal;
        
        SplitSignal(String signal) {
            this.signal = signal;
        }
    ...
    private static String wrap(String value, String pattern) {
        ...
        if (PFUtils.WRAP_BOTH.equals(pattern) || PFUtils.WRAP_LEFT.equals(pattern)) {
            value = "%" + value;
        }
        if (PFUtils.WRAP_BOTH.equals(pattern) || PFUtils.WRAP_RIGHT.equals(pattern)) {
            value = value + "%";
        }
