<%@page import="com.uk.sec.framework.system.model.*" %>
<%@ page contentType="text/html;charset=UTF-8" %>
<%@ include file="/commons/taglibs.jsp" %>
<rapid:override name="head">
    <title><%=SecSysRulevalue.TABLE_ALIAS%>查询</title>
</rapid:override>
<rapid:override name="container">
	<div class="positionPanel">
		<em></em>当前位置>><%=SecSysRulevalue.TABLE_ALIAS%>查询<a href="javascript:void(0)" onclick="window.history.go(-1)" class="back"></a>
	</div>
	<div class="formPanel">
		<form action="${ctx}/framework/system/SecSysRulevalue/list.do" method="post">
		<div class="formPanelHead">
			<ul>
				<li>查询</li>
			</ul>
		</div>
		<div class="formPanelBody">
			<table class="formTable" cellpadding="0" cellspacing="0">
        		<tr>
           			<td class="tdLabel"><%=SecSysRulevalue.ALIAS_RULE_KEYVALUE%>：</td>
           			<td>
                		<input  type="text" name="s_ruleKeyvalue" size="30" maxlength="3000" class=""/>
           			</td>
        		</tr>
        		<tr>
           			<td class="tdLabel"><%=SecSysRulevalue.ALIAS_SEQ_CURRENTVALUE%>：</td>
           			<td>
                		<input  type="text" name="s_seqCurrentvalue" size="30" maxlength="30" class=""/>
           			</td>
        		</tr>
        		<tr>
           			<td class="tdLabel"><%=SecSysRulevalue.ALIAS_RULE_ID%>：</td>
           			<td>
                		<input  type="text" name="s_ruleId" size="30" maxlength="32" class=""/>
           			</td>
        		</tr>
    		</table>
    	</div>
    	<div class="formPanelFoot">
    		<table cellpadding="0" cellspacing="0">
    			<tr>
    				<td>
    					<input type="submit" value="提交" onclick="return new Validation(document.forms[0]).validate();"/>
    					<input type="button" value="返回列表" onclick="window.location='${ctx}/framework/system/SecSysRulevalue/list.do'"/>
    					<input type="button" value="后退" onclick="history.back();"/>
    				</td>
    			</tr>
    		</table>
    	</div>
		</form>
	</div>
</rapid:override>
<%@ include file="/base.jsp"  %>