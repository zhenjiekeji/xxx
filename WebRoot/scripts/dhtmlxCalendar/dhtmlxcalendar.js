function dhtmlXCalendarObject(inps, skin) {
	...
	/*解决IE6下日历控件被select控件盖住的问题*/
	if(window.ActiveXObject&&!window.XMLHttpRequest){
		//如果为IE6则追加iframe
		var blankIframe = document.createElement("IFRAME");
		blankIframe.className = "dhtmlxcalendar_iframe";
		blankIframe.setAttribute("frameborder","0");
		blankIframe.setAttribute("scrolling","no");
		//blankIframe.setAttribute("allowtransparency","true");
		this.base.appendChild(blankIframe );
		//设置iframe透明
		var cWindow = blankIframe.contentWindow;
		cWindow.document.open();
		cWindow.document.writeln('<html><head>');
		cWindow.document.writeln('<style>body{background-color:#fff;}</style>');
		cWindow.document.writeln('</head><body></body></html>');
	...
	this.contDates.onclick = function(e){
		...
		if (t._date != null && !t._css_dis) {
			...
			if (that._activeInp && that.i[that._activeInp]) {
				...
				//为触发校验，添加焦点离开
				that.i[that._activeInp].focus();
				that.i[that._activeInp].blur();
			}
			// hide
	...
	this._show = function(inpId, autoHide) {
		...
		if (!inpId) {
			...
			if (this.pos == "right") {
				...
				basetop = this._getTop(this.i[inpId]);
			} else {
				this.base.style.left = this._getLeft(this.i[inpId])+"px";
				basetop = this._getTop(this.i[inpId])+this.i[inpId].offsetHeight-1;
			}
			this.base.style.top = basetop+"px";
